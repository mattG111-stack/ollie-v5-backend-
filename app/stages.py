...
def run_load(job_id: int, region: str = "Auckland") -> None:
    ...
    try:
        ...
        # low_memory=False: pandas otherwise reads the file in chunks and infers
        # each column's type per chunk, so a column that is numeric early and
        # text later gets different types in different parts of the SAME column.
        # That is the "Columns (76) have mixed types" warning, and it silently
        # produces wrong values rather than failing.
        df = pd.read_csv(job.file_path, on_bad_lines="skip", low_memory=False)
