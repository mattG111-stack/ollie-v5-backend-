    ...
    for _, r in df.iterrows():
        ...
        if not is_premium:
            if is_leasehold:
                ...
                # BARE LAND.
                #
                # The council valuation IS the land value for a section, and it
                # is PARCEL-specific — it already prices this lot's location,
                # zoning, contour and size. The suburb $/m2 rate is a median over
                # whatever sections happened to sell, which skews small and
                # urban; applied to a 6 ha block it produced $56,000k against a
                # $2,256k figure in the 28 July validation run.
                #
                # So when a CV exists it is the method, exactly as for every
                # other property type: CV x the SECTION sale/CV ratio for the
                # area (cv_ratio_for already resolves that per suburb and type —
                # measured at 0.954 for sections against 0.978 for houses).
                #
                # The previous fix bounded the land rate with min([land_val] +
                # caps). That stopped the explosions but only ever clips: where
                # the land rate lands BELOW the CV-based value, min() takes the
                # low number with no symmetric check, so sections that were
                # overvalued became systematically undervalued instead. A
                # one-sided guard is not an estimator.
                ...
                if cv_v:
                    ...
                    # The land rate is now a CROSS-CHECK, not the answer. A large
                    # disagreement means one of the two inputs is wrong — a stale
                    # CV, or a suburb rate built on unrepresentative sales — and
                    # the row is worth a human look. It does not silently move
                    ...
                    # No CV: the land rate is the only estimate available, and is
                    # trusted only on a small urban lot where the suburb rate is
                    # actually representative. A multi-hectare block with no CV
                    # cannot be sanity-checked at all.
