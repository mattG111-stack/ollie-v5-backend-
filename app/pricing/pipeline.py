...
from .glm import building_key, canonical_type, title_code
...
# Stale parent-lot CV detection (new builds on subdivided sites). The CV must
# exceed what same-size homes in the area actually sell for by this multiple
# before it is doubted at all — a genuinely superior house is not a data fault.
PARENT_LOT_CV_RATIO = 1.60
# ...and the asking price must sit at or below the market figure, i.e. the vendor
# agrees with the comps rather than with the CV. Both conditions together are
# what distinguishes a stale record from an expensive property.
PARENT_LOT_ASKING_BAND = 1.10
...
# Bare land: how far the suburb $/m2 land rate may sit from the CV-based value
# before the row is worth a human look. A cross-check threshold, not a clamp —
# exceeding it never moves the number.
SECTION_CROSSCHECK_TOLERANCE = 0.50
# A section whose CV implies more than this multiple of the suburb's land rate
# is carrying the CV of the title it was subdivided OUT of, not its own. 2.5x
# leaves room for a genuinely superior lot — corner, view, flat contour — while
# catching a lot that is priced as though it were several times its actual size.
SECTION_CV_PARENT_RATIO = 2.5
...
def run(for_sale_df: pd.DataFrame, sold: SoldDataset,
    ...
    for _, r in df.iterrows():
        ...
        # Placeholder asking: when the scraped "asking price" equals the CV to the
        # dollar, it's the scraper filling in CV for a by-negotiation listing —
        # not a real asking. The model says the area sells above CV, so these get
        # a fake ~60% margin. Treat the asking as not-real: keep the buy price
        # (still 95% of the listed figure) but suppress the deal flag/margin.
        asking_is_placeholder = bool(asking and cv and abs(asking - cv) < 0.005 * cv)
        ...
        is_leasehold = title_code(r.get("type_of_title")) == 2

        ollie_value = None
        anchor_fallback_used = False
        section_rate_disagrees = False
        cv_looks_parent_lot = False
        parent_lot_source = None
        if not is_premium:
            if is_leasehold:
                # LEASEHOLD — its own route, and the ONLY route for this tenure.
                #
                # Tenure trumps type here: a leasehold apartment is not an
                # apartment with a discount, it is a different asset. The buyer
                # acquires the lessee's interest while the council rates land and
                # improvements, so the pooled sale/CV ratio (~1.0, dominated by
                # freehold) values it as though it were freehold. That is how a
                # $79k Auckland Central apartment printed $673k at HIGH
                # confidence in the 28 July validation run.
                #
                # The ratio comes from leasehold sales only. Where there are too
                # few of those, this returns no value at all rather than a
                # guess — the discount is driven by ground rent and review dates
                # that this dataset does not carry, so a ratio built on three
                # sales would be fabricated, not merely weak.
                lh_ratio, lh_src = comp_engine.leasehold_ratio_for(
                    property_type=r.get("property_type"),
                    suburb=r.get("suburb"), district=r.get("district"))
                ollie_value = round(cv_v * lh_ratio) if (cv_v and lh_ratio) else None
                leasehold_source = lh_src
            elif ctype in ("Section", "Lifestyle Section"):
                # BARE LAND — the method is chosen per row, because CV and the
                # suburb $/m2 rate each fail badly in opposite regimes.
                #
                # CV fails on a NEWLY SUBDIVIDED lot. The council record lags the
                # survey plan, so a 400 m2 section still carries the CV of the
                # parent title it was cut from. CV x ratio then prices one lot at
                # the value of the whole block.
                #
                # The RATE fails on a large englobo block. It is a median over
                # sales of finished urban sections (typically 400-800 m2);
                # applied to 6.6 ha it produced $56,000k against a $2,256k asking
                # price (29 Annmarie Avenue). Raw englobo land trades far below
                # the per-m2 price of a titled section — that is the developer's
                # margin.
                #
                # Comparing the two exposes which one is describing this parcel:
                #
                #   implied = cv / land_area        ($/m2 the CV implies)
                #   ratio   = implied / suburb_rate
                #
                #   ratio >> 1  the CV covers far more land than this lot has —
                #               it is the parent title's. Use the rate.
                #   ratio << 1  the lot is far larger than the sections behind
                #               the rate — englobo. Use the CV.
                #   ratio ~ 1   they agree; either works, CV preferred as it is
                #               parcel-specific.
                rate = section_rates.rate_for(r.get("suburb"))
                land_val = round(land_v * rate) if (land_v and rate) else None

                cv_is_parent_title = False
                if cv_v and land_v and rate:
                    implied_rate = cv_v / land_v
                    rate_ratio = implied_rate / rate
                    cv_is_parent_title = rate_ratio > SECTION_CV_PARENT_RATIO

                if cv_is_parent_title and land_val is not None:
                    # The CV belongs to the pre-subdivision parcel. Price the lot
                    # on what land actually sells for per m2 around here.
                    ollie_value = land_val
                    section_rate_disagrees = True   # surface it for review
                elif cv_v:
                    ...
                    ollie_value = round(cv_v * ratio)
                    if land_val and ollie_value:
                        _gap = abs(land_val - ollie_value) / ollie_value
                        section_rate_disagrees = _gap > SECTION_CROSSCHECK_TOLERANCE
                elif land_val is not None and land_v and land_v <= 3000 and land_val <= 3_000_000:
                    # No CV: the rate is the only estimate available, and is
                    # trusted only on a small urban lot where it is actually
                    # representative. A multi-hectare block with no CV cannot be
                    # sanity-checked at all.
                    ollie_value = land_val
                elif asking_v:
                    ollie_value = round(asking_v)
                else:
                    ollie_value = None  # nothing to anchor on → show no number
            elif cv_v:
                ...
                # --- Stale parent-lot CV on a subdivided site --------------
                # A new house built on a subdivided section often still carries
                # the council valuation of the WHOLE original lot, because the
                # rating record has not caught up with the subdivision. The CV is
                # then describing land that is now three or four separate titles,
                # and every CV-anchored number inherits that — the valuation comes
                # out far above what the house is worth, the margin against asking
                # looks enormous, and it surfaces as the best deal on the list.
                #
                # The tell is that the ASKING PRICE and the SOLD DATA agree with
                # each other and the CV does not. A vendor pricing a new build
                # prices it against its neighbours; only the council record is
                # stale. So compare the CV against what same-size homes in the
                # area actually sell for: if the CV implies a value well above
                # that, while the asking price sits in line with it, the CV is
                # describing a different asset.
                #
                # Flagged, not silently corrected. The value is capped so it
                # cannot mint a fake deal, and the row is marked so the fault is
                # visible in review instead of hidden behind a plausible number.
                floor_v = _pos_num(r.get("key_floor_area"))
                _market = None
                _mkt_src = None
                _frate_detect = comp_engine.floor_rate_for(
                    suburb=r.get("suburb"), property_type=r.get("property_type"))
                if floor_v and _frate_detect:
                    _market = floor_v * _frate_detect
                    _mkt_src = "floor_rate"
                else:
                    _cap_detect = comp_engine.sold_price_cap(
                        suburb=r.get("suburb"), district=r.get("district"),
                        property_type=r.get("property_type"), beds=r.get("key_bedrooms"))
                    if _cap_detect:
                        _market = float(_cap_detect)
                        _mkt_src = "bed_count"

                if _market and cv_v and cv_v > _market * PARENT_LOT_CV_RATIO:
                    # The CV is out of line with the market. Does the asking price
                    # side with the market or with the CV? Only the former means a
                    # stale record rather than simply an expensive house.
                    if asking_v and asking_v <= _market * PARENT_LOT_ASKING_BAND:
                        cv_looks_parent_lot = True
                        # Re-anchor on what comparable homes actually sell for.
                        ollie_value = round(_market)
                        parent_lot_source = _mkt_src
        ...
        if ollie_value is not None:
            # LEASEHOLD: the CV is not a valid anchor. The council rates land and
            # improvements; the sale conveys only the lessee's interest, so a
            # leasehold legitimately trades at a fraction of CV. Anchoring on
            # max(CV, asking) snapped every leasehold back to ~0.95 x CV — which
            # is precisely the mechanism that produced $673k against a $79k
            # asking price. Anchor leasehold on the ASKING PRICE alone, and where
            # there is no asking, leave the tenure-specific value untouched.
            if is_leasehold:
                _anchor = asking_v or 0.0
            else:
        ...
        if cv_incomplete:
            ...
            expected_sale = round(mp) if mp else None
            expected_sale_path = f"matched_sold:{mtier}" if mp else "cv_incomplete"
            expected_sale_band = BAND_MATCHED if mp else None
            # Displayed value comes from the same source; no deal signal, because
            # a margin needs a value we can stand behind to ~5%.
            display_value = round(mp) if mp else None
            deal_value = None
        ...
        rows.append({
            ...
            # Stale parent-lot CV: the council record still describes the whole
            # pre-subdivision site. Written as cv_flag='suspect' so it lands in
            # the existing review gate (release._hold_reason already holds on it).
            "cv_flag": ("suspect" if cv_looks_parent_lot else None),
