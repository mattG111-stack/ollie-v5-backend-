...
# Leasehold legitimately trades far below CV — the council rates the whole
# property while the sale conveys only the lessee's interest. 0.3 threw those
# sales away as data errors; 0.05 keeps them.
RATIO_CV_LO_LEASEHOLD = 0.05
# Minimum leasehold sales behind a ratio before it is trusted at all. Below
# this the class is reported as unpriceable rather than guessed at.
LEASEHOLD_MIN_COMPS = 5
LAND_TOL = 0.15                 # ±15%
MIN_COMPS = 2
...
class CompEngine:
    ...
    def __init__(self, sold_df: pd.DataFrame):
        ...
        # Title class must be resolved BEFORE the guard is applied, because the
        # guard's lower bound is tenure-dependent.
        df["tclass"] = (df["type_of_title"].map(_title_bucket)
                        if "type_of_title" in df.columns else "FH")
        # A freehold selling at 0.2x CV is a broken council record. A LEASEHOLD
        # selling at 0.2x CV is an ordinary Tuesday — the council rates the land
        # and improvements while the buyer acquires only the lessee's interest,
        # net of a ground rent that can be tens of thousands a year. Applying the
        # freehold floor to leasehold discarded exactly the deepest-discount
        # sales, which are the most informative ones, and left the class looking
        # like it had almost no data. Bound leasehold far lower so those sales
        # survive into the median.
        lo = pd.Series(RATIO_CV_LO, index=df.index)
        lo[df["tclass"] == "LH"] = RATIO_CV_LO_LEASEHOLD
        df["sale_cv"] = cvr.where(
            (df["cv_numeric"] > 0) & (cvr >= lo) & (cvr <= RATIO_CV_HI))
        ...
        # Sample sizes behind the (type, tenure) medians. Needed so the leasehold
        # route can refuse to answer rather than quote a ratio built on noise.
        self._n_type_t = {k: int(v) for k, v in ttcnt.items()}
    ...
    def leasehold_ratio_for(self, *, property_type, suburb=None, district=None):
        """Sale/CV ratio measured over LEASEHOLD sales only.

        This is the sole valuation input for leasehold stock. The pooled ratio
        must never be used for it: pooling values a leasehold as though it were
        freehold, which is how a $79k Auckland Central apartment was priced at
        $673k.

        Resolves narrowest-first — (type, LH), then LH across all types — and
        returns (None, reason) when neither has enough sales behind it. Refusing
        to answer is correct here: a leasehold's discount is driven by ground
        rent and review dates that this data does not carry, so a ratio built on
        three sales is not a weak estimate, it is a fabricated one.

        Returns (ratio, source).
        """
        ctype = canonical_type(property_type)
        key = (ctype, "LH")
        n_typed = self._n_type_t.get(key, 0)
        if key in self._ratio_type_t and n_typed >= LEASEHOLD_MIN_COMPS:
            return float(self._ratio_type_t[key]), f"leasehold:{ctype}:n={n_typed}"
        n_all = self._n_tclass.get("LH", 0)
        if "LH" in self._ratio_tcls and n_all >= LEASEHOLD_MIN_COMPS:
            return float(self._ratio_tcls["LH"]), f"leasehold:all_types:n={n_all}"
        return None, f"leasehold:insufficient:n={n_all}"
