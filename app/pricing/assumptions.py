...
# Carparks and storage units. Already caught incidentally by the "other -"
# prefix above, but listed explicitly so the exclusion survives a feed that
# renames the category — and so the intent is legible rather than accidental.
CARPARK_TYPE_KEYWORDS = ("carpark", "car park", "car-park", "parking", "storage unit")


# === Location routing ===
# Hauraki Gulf islands. Ferry-only markets with their own price dynamics and
# very few sales — comp-matching them against mainland Auckland produces
# confident nonsense in both directions. Named explicitly rather than matching
# on "island" because Herald Island, Hobsonville and Te Atatu Peninsula are
# road-accessible mainland suburbs that must NOT be caught.
ISLAND_KEYWORDS = (
    "waiheke", "great barrier", "aotea island", "kawau", "rakino",
    "ponui", "rotoroa", "motutapu", "pakatoa", "hauraki gulf islands",
)
# Explicitly mainland despite the name — road or causeway accessible.
ISLAND_EXCEPTIONS = ("herald island", "beach haven island")
...
def is_carpark_type(property_type: str | None) -> bool:
    if not property_type:
        return False
    s = str(property_type).lower()
    return any(k in s for k in CARPARK_TYPE_KEYWORDS)


def is_island_location(suburb: str | None, district: str | None = None) -> bool:
    """True for Hauraki Gulf island stock. Checks the exception list first so a
    road-accessible suburb with 'island' in its name is never caught."""
    blob = " ".join(str(x).lower() for x in (suburb, district) if x)
    if not blob.strip():
        return False
    if any(x in blob for x in ISLAND_EXCEPTIONS):
        return False
    return any(k in blob for k in ISLAND_KEYWORDS)
