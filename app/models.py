...
class IngestJob(Base):
    ...
    # ---- four-stage pipeline (load → enrich → price → publish) ----------------
    # Which stage this job ran. 'legacy' marks jobs from the old welded ingest.
    stage_name: Mapped[str | None] = mapped_column(String(16), index=True)
    # Durable progress. These are written to the DB as the stage runs, NOT held
    # in the browser, so closing the tab or refreshing loses nothing and a job
    # that outlives its HTTP request is still observable.
    rows_processed: Mapped[int] = mapped_column(Integer, default=0, nullable=False)
    rows_filled: Mapped[int] = mapped_column(Integer, default=0, nullable=False)
    rows_missed: Mapped[int] = mapped_column(Integer, default=0, nullable=False)
    rows_skipped: Mapped[int] = mapped_column(Integer, default=0, nullable=False)
    # Co-operative cancellation: the endpoint sets this, the worker polls it.
    # Threads can't be killed safely mid-transaction, so we ask rather than kill.
    cancel_requested: Mapped[bool] = mapped_column(Boolean, default=False, nullable=False)
    # Touched every batch of rows. status='running' with a stale heartbeat means
    # the process died without writing a terminal state (a Railway redeploy, say)
    # — which is exactly the case a percentage bar alone cannot tell you about.
...
class PropertyForSale(_PropertyMixin, Base):
    ...
    # ---- four-stage pipeline, per-row state -----------------------------------
    # Set by the ENRICH stage. Recorded per row so a re-run resumes instead of
    # repeating CoreLogic calls already made — the whole point of a stage you can
    # restart after it dies at 60%.
    #   pending  — not yet looked up
    #   filled   — CoreLogic returned data and at least one blank was filled
    #   missed   — CoreLogic returned nothing. NOT a failure: plenty of addresses
    #              simply aren't resolvable, and the row still prices on what it has
    #   skipped  — needed no lookup (floor and land already present)
    enrich_status: Mapped[str] = mapped_column(String(16), default="pending", nullable=False, index=True)
    enrich_cells_filled: Mapped[int] = mapped_column(Integer, default=0, nullable=False)
    enriched_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True))
    # Set by the PRICE stage. NULL means loaded but never valued, which is a
    # legitimate resting state under staging and must never reach a live view.
