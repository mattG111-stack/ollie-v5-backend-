...
def _run_job(job_id: int, region: str) -> None:
    ...
    try:
        ...
        if job.batch_type == BatchType.SOLD.value:
            ...
            # Fill blank floor/land from CoreLogic BEFORE pricing so the valuation
            # is computed on the best numbers — then hold what's still bad.
            _update(db, job_id, stage="filling gaps from CoreLogic + pricing", progress_pct=30)
            r = ingest.ingest_for_sale(db, df, sold_df, job.filename, region=region,
                                       uploaded_by_id=job.uploaded_by_id, publish=False, fill_missing=True)
