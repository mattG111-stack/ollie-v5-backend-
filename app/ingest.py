...
# ---- Reject rules, split by whether ENRICH could still rescue the row --------
# The old welded ingest applied every rule at write time, which was only correct
# because CoreLogic had already run in the same function. Once the stages come
# apart that stops holding: a row with no floor area is exactly the row enrich
# exists to fix, so rejecting it at LOAD would delete the evidence before the
# stage that repairs it ever runs.
#
# HARD  — nothing CoreLogic returns can change the verdict. Reject at load.
# SOFT  — depends on a field enrich can fill. Deferred to PRICE, and applied
#         there as a HOLD rather than a delete, so the row stays inspectable.

def _hard_reject_reason(payload: dict, asking, *, region: str) -> str | None:
    """Applied at LOAD. Only rules that no amount of enrichment can flip."""
    # Commercial, industrial and retirement stock. The product is comp-priced
    # residential; these have no comparable residential market to price against,
    # and — worse — their council record often describes a different asset than
    # the one being sold. A "Commercial - Elderly" listing carries the CV of the
    # entire retirement village while the listing is a single unit, so valuing it
    # at CV produced $45,000k against a $487k asking price. That is a category
    # error, not an inaccurate estimate, and no lookup fixes it.
    #
    # Rejected at LOAD rather than at price so they never reach the enrich stage:
    # a CoreLogic call costs 0.5s, and spending it on stock that can never be
    # published is time and rate-limit budget thrown away.
    if A.is_commercial_type(payload.get("property_type")):
        return "commercial_type"
    # Carparks and storage units — not dwellings, nothing to comp against.
    if A.is_carpark_type(payload.get("property_type")):
        return "carpark_type"
    # Hauraki Gulf islands. A ferry-only market with its own dynamics and very
    # few sales; comp-matching Waiheke against mainland Auckland is confident
    # nonsense in both directions.
    if A.is_island_location(payload.get("suburb"), payload.get("district")):
        return "island_location"
    # Our sold dataset is Auckland-only; comp-matching a Christchurch listing
    # against it produces garbage no lookup can repair.
    if payload.get("region") and payload["region"] != region:
        return "non_target_region"
    # Without a suburb there is nothing to comp-match against, and CoreLogic is
    # queried BY suburb, so it cannot supply one either.
    if not payload.get("suburb"):
        return "no_suburb"
    # Placeholder asking ($1, $2) from "by negotiation" listings — the source of
    # the "underpriced by 1,500,000x" outliers.
    if asking is not None and asking < 10_000:
        return "placeholder_asking"
...
def _soft_reject_reason(payload: dict, asking, *, property_type=None) -> str | None:
    """Applied at PRICE, after enrich has had its chance. Each of these turns on
    a field (CV, floor area) that CoreLogic can supply, so they are only fair to
    apply once the enrich stage has run or been deliberately skipped."""
    cv = payload.get("cv_numeric")
    # Every valuation method we have is anchored on the council valuation.
    if not cv:
        return "no_cv"
    # Asking and CV wildly apart: one of the two published figures is simply
    # wrong and there is no way to tell which.
    if asking and cv and abs(float(asking) - float(cv)) > 0.50 * float(cv):
        return "asking_vs_cv_50pct"
    # A building with no floor area can't be size-valued. Bare land legitimately
    # has none and is valued off the suburb's $/m2 rate.
    if payload.get("floor_area_m2") is None and not A.is_vacant_type(property_type):
        return "dwelling_no_floor"
    return None


# ---- Stage 1: LOAD (raw rows to the DB, no external calls, no pricing) -------
def load_for_sale(
    db: Session,
    ...
    *,
    ...
    """Parse a for-sale CSV into staged rows and nothing more.

    No CoreLogic. No pricing pipeline. No archiving of the live batch. This is
    deliberately the boring stage: it should finish in seconds on a 138 MB file
    so the operator can look at what actually arrived before spending an hour of
    CoreLogic calls on it.

    Rows land with enrich_status='pending' and priced_at=NULL — loaded, visible
    in the admin grid, and not live. Valuation columns are left empty; the price
    stage fills them.
    """
    for_sale_df, dropped = _dedupe_by_slug(for_sale_df)
    batch = ImportBatch(
        batch_type=BatchType.FOR_SALE.value, region=region, filename=filename,
        rows_total=len(for_sale_df), is_active=False, status="staged",
        uploaded_by_id=uploaded_by_id,
        note=(f"deduped {dropped} duplicate slug_ids" if dropped else None),
    )
    db.add(batch); db.flush()
    ...
    for _, row in for_sale_df.iterrows():
        rowd = row.to_dict()
        payload = _common_property_payload(rowd, region=region)
        asking = _to_float(rowd.get("price_numeric"))

        reason = _hard_reject_reason(payload, asking, region=region)
        if reason:
            rejected += 1
        ...
        rec = PropertyForSale(
            **payload,
            import_batch_id=batch.id,
            asking_price=asking,
            listing_type=_detect_listing_type(_to_str(rowd.get("price_display")), asking),
    ...
    note_parts = []
    if rejected_reasons:
        note_parts.append("rejected: " + ", ".join(f"{k}={v}" for k, v in rejected_reasons.items()))
    if note_parts:
        batch.note = (batch.note or "") + (" · " if batch.note else "") + " · ".join(note_parts)
    db.commit()
    return IngestResult(BatchType.FOR_SALE.value, batch.id, inserted, rejected, batch.note or "")


# ---- For-sale ingestion (runs the full pricing pipeline) ----
# NOTE: retained for the one-shot path and for scripts/. The weekly admin flow
# now goes through app/stages.py, which runs load / enrich / price separately.
def ingest_for_sale(
    db: Session,
    for_sale_df: pd.DataFrame,
    ...
    for _, row in enriched.iterrows():
        ...
        rec = PropertyForSale(
            **payload,
            ...
            cv_flag=_to_str(row.get("cv_flag")),
