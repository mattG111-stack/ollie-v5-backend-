...
def test_commercial_is_rejected_at_load(db):
    """Commercial / industrial / retirement stock never enters the system.

    The worst row in the 28 July validation report was a 'Commercial - Elderly'
    listing valued at $45,000k against a $487k asking price, because the CV on
    the record describes the whole retirement village rather than the unit for
    sale. Nothing downstream can repair that, so it is refused at the door.
    """
    from app import ingest
    from app.models import PropertyForSale

    df = _csv(6)
    df.loc[0, "property_type"] = "Commercial - Elderly"
    df.loc[1, "property_type"] = "Commercial - Retail use"
    df.loc[2, "property_type"] = "Industrial"
    r = ingest.load_for_sale(db, df, "test.csv")

    assert r.rows_inserted == 3
    assert r.rows_rejected == 3
    assert "commercial_type=3" in r.notes

    kept = db.query(PropertyForSale).filter(
        PropertyForSale.import_batch_id == r.batch_id).all()
    assert all("ommercial" not in (p.property_type or "") for p in kept)


def test_commercial_never_reaches_enrich(db):
    """Rejected at load means no CoreLogic call is ever spent on them."""
    from app import ingest, stages

    df = _csv(4)
    df.loc[0, "property_type"] = "Commercial - Office"
    r = ingest.load_for_sale(db, df, "test.csv")

    cov = stages.enrich_coverage(db, r.batch_id)
    assert cov["total"] == 3, "a commercial row entered the enrich candidate set"


def test_residential_types_are_untouched(db):
    """The filter must not catch ordinary housing stock."""
    from app import ingest

    df = _csv(5)
    for i, t in enumerate(["House", "Townhouse", "Unit", "Residential - Apartments",
                           "Lifestyle Section"]):
        df.loc[i, "property_type"] = t
    r = ingest.load_for_sale(db, df, "test.csv")
    assert r.rows_inserted == 5, f"a residential type was rejected: {r.note}"


def test_delete_batch_removes_rows_and_jobs(db):
    """Clearing old import history must take the listings with it, or the rows
    are orphaned and still counted by anything scanning the table."""
    from app import ingest, stages
    from app.models import IngestJob, PropertyForSale

    r = ingest.load_for_sale(db, _csv(5), "old.csv")
    stages.new_job(db, stage_name=stages.LOAD, batch_type="for_sale",
                   filename="old.csv", batch_id=r.batch_id)

    assert db.query(PropertyForSale).filter(
        PropertyForSale.import_batch_id == r.batch_id).count() == 5

    from app.models import ImportBatch, PropertyRent, PropertySold
    b = db.get(ImportBatch, r.batch_id)
    for model in (PropertyForSale, PropertySold, PropertyRent):
        db.query(model).filter(model.import_batch_id == r.batch_id).delete(
            synchronize_session=False)
    db.query(IngestJob).filter(IngestJob.batch_id == r.batch_id).delete(
        synchronize_session=False)
    db.delete(b)
    db.commit()

    assert db.get(ImportBatch, r.batch_id) is None
    assert db.query(PropertyForSale).filter(
        PropertyForSale.import_batch_id == r.batch_id).count() == 0
    assert db.query(IngestJob).filter(IngestJob.batch_id == r.batch_id).count() == 0


def test_live_batch_is_protected_from_deletion(db):
    """Deleting the published batch would empty the site for every user."""
    from app.models import ImportBatch

    b = ImportBatch(batch_type="for_sale", region="Auckland", filename="live.csv",
                    is_active=True, status="published")
    db.add(b); db.commit()

    # The guard the endpoint applies.
    assert b.is_active or b.status == "published", "live batch would be deletable"
